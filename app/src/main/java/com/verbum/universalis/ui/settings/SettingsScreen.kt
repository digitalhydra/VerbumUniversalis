    ...
    Scaffold(
        ...
        Column(
            ...
            Button(
                onClick = {
                    scope.launch {
                        val authManager = com.verbum.universalis.data.oauth.OAuthManager()
                        val result = authManager.startDeviceFlow()
                        if (result is com.verbum.universalis.data.oauth.DeviceFlowResult.Success) {
                            ...
                            launch {
                                while (polling) {
                                    ...
                                    if (tokenResult is com.verbum.universalis.data.oauth.TokenResult.Success) {
                                        token = tokenResult.accessToken
                                        polling = false
                                    } else if (tokenResult is com.verbum.universalis.data.oauth.TokenResult.Error) {
                                        polling = false
                                    }
                                }
                            }
                ...
                onClick = {
                    scope.launch {
                        ...
                        if (repo != null) {
                            ...
                            gitSyncViewModel.configureRepo(repoUrl, token)
                            // Upload SSH key
                            val sshManager = com.verbum.universalis.data.ssh.SSHKeyManager(androidx.compose.ui.platform.LocalContext.current)
